(function() {
  var ColorPicker, DrawingCanvas, Widget, main, widget;

  DrawingCanvas = window.DrawingCanvas = function(id) {
  ...
  ColorPicker = window.ColorPicker = function(id) {
    var pickerImg;
  ...
  ColorPicker.prototype.pickColor = function(e) {
    var imgData, rgbArray, x, y;
    x = e.clientX - (this.colorPickerCanvas.getBoundingClientRect().left);
    y = e.clientY - (this.colorPickerCanvas.getBoundingClientRect().top);
    imgData = this.colorPickerContext.getImageData(x, y, 1, 1).data;
  ...
  Widget = function() {
    ...
    this.widgetWidth = this.drawingWidgetElement.getAttribute('width') || window.innerWidth * 9 / 10;
    this.widgetHeight = this.drawingWidgetElement.getAttribute('height') || window.innerHeight * 9 / 10;
  };
  ...
  Widget.prototype.setUpDrawingWidget = function() {
    this.setSize(this.drawingWidgetElement, this.widgetWidth, this.widgetHeight);
  };

  Widget.prototype.setUpDrawingCanvas = function() {
    this.drawingCanvasElement.setAttribute('width', this.widgetWidth);
  ...
  Widget.prototype.setUpColorPickerContainer = function() {
    this.setSize(this.colorPickerContainer, this.widgetWidth * 1 / 10, this.widgetHeight * 4 / 5);
  ...
  Widget.prototype.setUpColorPicker = function() {
    this.colorPickerElement.setAttribute('width', this.widgetWidth * 1 / 10);
    this.colorPickerElement.setAttribute('height', this.widgetHeight * 4 / 5);
    this.colorPickerContainer.style.position = 'absolute';
    this.colorPickerElement.id = 'color-picker';
  };

  Widget.prototype.setUpColorSample = function() {
    this.setSize(this.colorSampleElement, this.widgetWidth * 1 / 10, this.widgetHeight * 1 / 5);
    this.colorSampleElement.style.background = 'black';
    this.colorSampleElement.style.position = 'absolute';
    this.colorSampleElement.style.bottom = '-80';
  ...
  Widget.prototype.initializeColorCanvas = function() {
    ...
    this.colorPickerElement.addEventListener('mousedown', (function(e) {
      var newColor;
      this.changingColor = true;
      newColor = this.colorPicker.pickColor.bind(this.colorPicker, e)();
      this.drawingCanvas.setColor.bind(this.drawingCanvas, newColor)();
      this.colorSampleElement.style.background = newColor;
    }).bind(this), true);
    this.colorPickerElement.addEventListener('mouseup', (function(e) {
      this.changingColor = false;
    }).bind(this), true);
    this.colorPickerElement.addEventListener('mousemove', (function(e) {
      var newColor;
    ...
    this.saveImageElement.addEventListener('click', (function(e) {
      var imagePNG;
      imagePNG = this.drawingCanvas.getPNG.bind(this.drawingCanvas)();
      window.open(imagePNG);
    }).bind(this), true);
    this.clearImageElement.addEventListener('click', (function(e) {
      var imagePNG;
      imagePNG = this.drawingCanvas.hardReset.bind(this.drawingCanvas)();
    }).bind(this), true);
  };
